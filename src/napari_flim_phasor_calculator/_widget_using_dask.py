...
def make_flim_phasor_plot(image_layer : Image,
    ...
    # Does not work if g, s and space mask are dask arrays!
    ...
    # #####Using Dask #########
    # Using ravel doesnot get proper size
    # phasor_components = {'label': da.ravel(label_image[space_mask]), 
    #                      'G': da.ravel(g[space_mask]),
    #                      'S': da.ravel(s[space_mask])}
    # One must 'compute_chunk_sizes' after masking and before adding to table
    # g_space_masked = g[space_mask]
    # g_space_masked.compute_chunk_sizes()
    # s_space_masked = s[space_mask]
    # s_space_masked.compute_chunk_sizes()
    # label_space_masked = label_image[space_mask]
    ############# Adding to table
    # phasor_components = {'label': label_space_masked, 
    #                      'G': g_space_masked,
    #                      'S': s_space_masked}   
    # table = pd.DataFrame(phasor_components)
    ############ Another option may be to use dask.dataframe
    # But not sure if it will work (https://docs.dask.org/en/stable/generated/dask.dataframe.from_dask_array.html)
    # Something like this:
    # dask_stack = da.stack([label_space_masked, g_space_masked, s_space_masked])
    # table = dd.io.from_dask_array(dask_stack, columns=['label', 'g', 's'])
