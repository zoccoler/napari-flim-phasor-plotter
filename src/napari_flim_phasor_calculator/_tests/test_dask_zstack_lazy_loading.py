...
folder_path = r"C:\Users\mazo260d\Desktop\Conni_BiA_PoL\output_as_tif"
...
def read_tif_3D_data(file_paths, image_slice_max_shape):
...
@dask.delayed
def read_tif_data_3D_delayed(file_paths, image_slice_shape):
    return read_tif_3D_data(file_paths, image_slice_shape)

# Read all slices to get slice max shape and dtype
slice_shape_list = []
...
file_paths = [file_path for file_path in folder_path.iterdir() if file_path.suffix == '.tif']
# image_3D = read_tif_3D_data(file_paths, image_slice_max_shape=slice_max_shape)
        ...
        if file_path.suffix == '.tif':
            ...
            if current_t is not None:
                if current_t > previous_t:
                    z_stack = read_tif_3D_data(z_path_list, image_slice_max_shape = image_stack_shape[1:])
